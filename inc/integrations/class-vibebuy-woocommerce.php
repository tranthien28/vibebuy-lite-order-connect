		...
		foreach ($active_channels as $channel_id) {
			...
			if (is_product()) {
				...
				if ($_product) {
					...
					if ('instock' === $stock_condition && !$_product->is_in_stock())
						continue;
					if ('outofstock' === $stock_condition && $_product->is_in_stock())
						continue;
				}
			}
	...
	public function add_product_context($data)
	{
		...
		if ($_product) {
			...
			$data['product'] = array(
				...
				'formatted_price' => strip_tags(wc_price($_product->get_price())),
			...
			if ($is_variable) {
				...
				foreach ($variation_ids as $var_id) {
					...
					$data['product']['variations'][] = array(
						...
						'formatted_price' => strip_tags(wc_price($variation->get_price())),
						'image' => get_the_post_thumbnail_url($variation->get_id(), 'medium') ?: '',
